import { STORAGE_DEFAULT_VALUES, type StorageName, type StorageSchemas } from '../../common/storage'
...
function createStore<T extends StorageName>(name: T, initValue: StorageSchemas[T]) {
  ...
  $effect.root(() => {
    $effect(() => {
      const value = $state.snapshot(currentValue) //  $state.snapshot是触发effect必须的
